...
func main() {
    ...
    peakMemoryThreshold := flag.Float64("U", 6.0, "Warn if peak memory usage is above threshold")
    ...
    if wpStatus.PluginUpdateCount > *pluginUpdateThreshold {
        outputMessage += "Plugin updates exceed threshold "
        exitCode = 1
    }
    if wpStatus.ThemeUpdateCount > *themeUpdateThreshold {
        outputMessage += "Theme updates exceed threshold "
        exitCode = 1
    }
    ...
    if wpStatus.UnapprovedComments > *unapprovedCommentsThreshold {
        outputMessage += "Unapproved comments exceed threshold "
        exitCode = 1
    }
    if wpStatus.ResponseTimeMs > *responseTimeThreshold {
        outputMessage += "Response time exceeds threshold "
        exitCode = 1
    }
    if wpStatus.PeakScriptMemoryMb > *peakMemoryThreshold {
        outputMessage += "Memory exceeds threshold "
        exitCode = 1
    }
